...
Equations lookup_tc_var ctx tys (tv: ty_valu ctx tys) (tc_var: ty_ctx_var ctx) : Type := {
  ...
  (* lookup_tc_var _ (TCVThere c _ tc_var) := lookup_tc_var c tc_var; *)
}.
...
Equations interp_tm t_ctx tys tv e_ctx e (env: valu t_ctx tys tv e_ctx) (t: tm sig_sorts t_ctx e_ctx e) : interp_te t_ctx tys tv 0 e := {
  interp_tm _ _ _ _ _ env (TVar _ _ v) := interp_var _ _ _ _ _ env v;
}.

Equations interp_fm t_ctx tys tv e_ctx (env: valu t_ctx tys tv e_ctx) (f: fm sig_sorts t_ctx e_ctx) : Prop := {
...
Equations interp_pfm t_ctx tys (tv: ty_valu t_ctx tys) (pf: poly_fm sig_sorts t_ctx) : Prop := {
  interp_pfm t_ctx tys tv (PFm _ _ f) := 
    interp_fm t_ctx tys tv _ (VEmp _ _ _) f;
  interp_pfm t_ctx tys tv (PForall _ _ pf) := 
    forall (T: Type), 
